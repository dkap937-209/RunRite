...
class TrackerViewModel(
    ...
    private val hasBodySensorPermissions = MutableStateFlow(false)
    ...
    init {
        phoneConnector
            ...
            .onEach { connectedNode ->
                state = state.copy(
                    isConnectedPhoneNearby = connectedNode.isNearby == true
                )
            }
    ...
    fun onAction(action: TrackerAction, triggeredOnPhone: Boolean = false) {
        ...
        when(action) {
            is TrackerAction.OnBodySensorPermissionResult -> {
                hasBodySensorPermissions.value = action.isGranted
